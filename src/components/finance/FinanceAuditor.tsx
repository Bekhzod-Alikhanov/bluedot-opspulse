...
export default function FinanceAuditor({ halted }: { halted: boolean }) {
  ...
  const copy = async () => {
    try { await navigator.clipboard.writeText(ESCALATION); setCopied(true); setTimeout(() => setCopied(false), 2000); } catch {}
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mt-4 overflow-hidden rounded-xl border border-slate-800">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-900/70 font-mono text-[10px] uppercase tracking-widest text-slate-500">
              <tr><th className="px-4 py-2.5">Line item</th><th className="px-4 py-2.5 text-right">Seats</th><th className="px-4 py-2.5 text-right">Unit</th><th className="px-4 py-2.5 text-right">Amount</th></tr>
            </thead>
      ...
      <section className="panel animate-fade-up rounded-2xl p-5">
        ...
        <pre className="mt-3 max-h-72 overflow-y-auto whitespace-pre-wrap rounded-xl border border-slate-800 bg-slate-950/60 p-4 font-mono text-[12px] leading-relaxed text-slate-300">{ESCALATION}</pre>
